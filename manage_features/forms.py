...
class Ar_Featurre_Form(forms.ModelForm):
    ...
    def __init__(self,org_info,*args, **kwargs):
        ...
        self.fields['CE_ID'] = forms.ModelChoiceField(queryset=AR_EPIC_CAPABILITY.objects.filter(ORG_ID__in=Subquery(org_info.values("id"))), widget=forms.Select(attrs={"class": "form-control"}))
        self.fields['User_story'] = forms.ModelChoiceField(queryset=AR_USER_STORY.objects.filter(ORG_id__in=Subquery(org_info.values("id"))), widget=forms.Select(attrs={"class": "form-control","required":"false"}))
