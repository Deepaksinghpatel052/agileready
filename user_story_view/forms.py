...
class Ar_User_Story_Form(forms.ModelForm):
    ...
    class Meta:
        model = AR_USER_STORY
        fields = ['title','owner', 'story_tri_part_text', 'acceptance_criteria','BV_ID', 'ac_readability_score','conversation', 'convo_readability_score', 'attachments','autoscoring_on', 'archive_indicator','readiness_quality_score', 'story_points', 'user_story_status','UST_ID','ar_user','backlog_parent']
    ...
    def __init__(self, user_id,org_info, *args, **kwargs):
        ...
        self.fields['BV_ID'] = forms.ModelChoiceField(required=False,empty_label="Please select Business Value or None",queryset=AR_BUSINESS_VALUE.objects.filter(ORG_ID=org_info),widget=forms.Select(attrs={"class": "form-control"}))
