...
class AR_FEATURE(models.Model):
    ...
    create_dt = models.DateTimeField(default=datetime.now(), blank=True)
    update_by = models.IntegerField(default="",blank=True)
    update_dt = models.DateTimeField(default=datetime.now(),blank=True)
    def __str__(self):
...
class AR_USER_STORY(models.Model):
    ...
    updated_dt = models.DateTimeField(default=datetime.now(),blank=True)
