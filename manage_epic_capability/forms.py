...
class Ar_Epic_Capability_Form(forms.ModelForm):
    ...
    class Meta:
        ...
        # fields = ['Cepic_key', 'Cepic_desc', 'Children_feature_list']
        fields = ['Cepic_key', 'Cepic_desc','PROJECT_ID']
    def __init__(self,org_info,*args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['PROJECT_ID'] = forms.ModelChoiceField(queryset=AR_product.objects.filter(ORG_ID__in=Subquery(org_info.values("id"))), widget=forms.Select(attrs={"class": "form-control"}))
