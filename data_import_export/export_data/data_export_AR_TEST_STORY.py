...
from test_story_view.models import AR_TEST_STORY,test_story_file_attachment
...
def get_AR_TEST_STORY_data(org_ins,worksheet):
    if AR_TEST_STORY.objects.filter(ORG_id=org_ins).exists():
        data = AR_TEST_STORY.objects.filter(ORG_id=org_ins)
        ...
        worksheet.write('F1', "scenario_parent")
        ...
        worksheet.write('L1', "test_story_status")
        ...
        for rows in data:
            ...
            worksheet.write('F' + str(i), str(rows.scenario_parent))
            ...
            worksheet.write('L' + str(i), str(rows.test_story_status))
            ...
            set_attachment_url = ''
            if test_story_file_attachment.objects.filter(test_story=rows).exists():
                get_attachment = test_story_file_attachment.objects.filter(test_story=rows)
...
def get_AR_TEST_STORY_data_CSV(org_ins, file_name):
    if AR_TEST_STORY.objects.filter(ORG_id=org_ins).exists():
        data = AR_TEST_STORY.objects.filter(ORG_id=org_ins)
        with open(file_name, mode='w') as data_file:
            file_writer = csv.writer(data_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            file_writer.writerow([ "title","story_tri_part_text","acceptance_criteria","ac_readability_score","conversation","scenario_parent","convo_readability_score","autoscoring_on","archive_indicator","readiness_quality_score","story_points","test_story_status","User_story_type","ar_user","owner","created_by","created_dt","updated_dt","updated_by","organization_name","attachments"])
            for rows in data:
                set_attachment_url = ''
                if test_story_file_attachment.objects.filter(test_story=rows).exists():
                    get_attachment = test_story_file_attachment.objects.filter(test_story=rows)
                    for item in get_attachment:
                        if item.attachment:
                            set_attachment_url  +=str(item.attachment.url)+"&"
                file_writer.writerow([str(rows.title), str(rows.story_tri_part_text),str(rows.acceptance_criteria),str(rows.ac_readability_score),str(rows.conversation),str(rows.scenario_parent),str(rows.convo_readability_score),str(rows.autoscoring_on),str(rows.archive_indicator),str(rows.readiness_quality_score),str(rows.story_points),str(rows.test_story_status),str(rows.UST_ID),str(rows.ar_user),str(rows.owner),str(rows.created_by),str(rows.created_dt),str(rows.updated_dt),str(rows.updated_by),str(org_ins),str(set_attachment_url)])
    return True


def get_AR_TEST_STORY_data_CSV2(request,org_ins, file_name):
    ...
    if request.POST["scenario_list"] == "":
        pass
    else:
        scenario_list = request.POST["scenario_list"]

        if scenario_list == "none":
            filters1 = filters1 & Q(scenario_parent__scenario_slug__exact=scenario_list) | Q(
                scenario_parent__scenario_slug__isnull=True)
        else:

            filters1 = filters1 & Q(scenario_parent__scenario_slug__exact=scenario_list)

        # filters = filters & Q(scenario_parent__scenario_slug__exact=scenario_list)
    ...
    if request.POST["business_value_list"] == "":
        ...
        # filters = filters & Q(BV_ID__business_value_slug__exact=business_value_list)
    ...
    if request.POST["user_status_list"] == "":
        ...
        if user_status_list == "none":
            filters3 = filters3 & Q(test_story_status__user_status_slug__exact=user_status_list) | Q(
                test_story_status__user_status_slug__isnull=True)
        else:

            filters3 = filters3 & Q(test_story_status__user_status_slug__exact=user_status_list)


        # filters = filters & Q(test_story_status__user_status_slug__exact=user_status_list)
    ...
    if request.POST["user_type_list"] == "":
        ...
        # filters = filters & Q(UST_ID__user_type_slug__exact=user_type_list)

    if AR_TEST_STORY.objects.filter(filters1).filter(filters2).filter(filters3).filter(filters4).exists():
        data = AR_TEST_STORY.objects.filter(filters1).filter(filters2).filter(filters3).filter(filters4).order_by("-id").filter(~Q(title='None'))
        with open(file_name, mode='w') as data_file:
            file_writer = csv.writer(data_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            file_writer.writerow([ "title","story_tri_part_text","acceptance_criteria","ac_readability_score","conversation","scenario_parent","convo_readability_score","autoscoring_on","archive_indicator","readiness_quality_score","story_points","test_story_status","User_story_type","ar_user","owner","created_by","created_dt","updated_dt","updated_by","organization_name","attachments"])
            for rows in data:
