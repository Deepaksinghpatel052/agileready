...
def get_AR_USER_STORY_data(org_ins,worksheet):
    if AR_USER_STORY.objects.filter(ORG_id=org_ins).exists():
        ...
        for rows in data:
            ...
            set_attachment_url = ''
            if file_attachment.objects.filter(user_story=rows).exists():
                get_attachment = file_attachment.objects.filter(user_story=rows)
...
def get_AR_USER_STORY_data_CSV2(request,org_ins, file_name):
    ...
    if request.POST["user_status_list"] == "":
        ...
        if user_status_list == "none":
            filters3 = filters3 & Q(user_story_status__user_status_slug__exact=user_status_list) | Q(
                user_story_status__user_status_slug__isnull=True)
        else:
            filters3 = filters3 & Q(user_story_status__user_status_slug__exact=user_status_list)
    ...
    if AR_USER_STORY.objects.filter(filters1).filter(filters2).filter(filters3).filter(filters4).exists():
        data = AR_USER_STORY.objects.filter(filters1).filter(filters2).filter(filters3).filter(filters4).order_by("-id").filter(~Q(title='None'))
        with open(file_name, mode='w') as data_file:
            file_writer = csv.writer(data_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            file_writer.writerow([ "title","story_tri_part_text","acceptance_criteria","ac_readability_score","conversation","backlog_parent","convo_readability_score","autoscoring_on","archive_indicator","readiness_quality_score","story_points","user_story_status","User_story_type","ar_user","owner","created_by","created_dt","updated_dt","updated_by","organization_name","attachments"])
            for rows in data:
