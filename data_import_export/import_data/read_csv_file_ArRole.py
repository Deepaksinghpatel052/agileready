...
def read_ArRole_csv(request,set_data_type,file_ins,org_ins,file_name_value,file_name_txt,user_id,set_dummy_data=""):
        ...
        for item in file_name:
            ...
            if item[1] == "":
                ...
                if ArRole.objects.filter(title=item[1]).filter(ORG_ID=org_ins).exists():
                    ...
                    add_ar_role = ArRole(title=item[1],data_Type=set_dummy_data,desc=item[2],use=item[3],ORG_ID=org_ins,create_by=created_by_ins,create_dt=created_dt,update_by=updateded_by_ins,update_dt=updated_dt)
                    try:
                        add_ar_role.save()
    ...
    exists_message = ""
    if empty_data > 0:
        empty_mess = str(empty_data) + " Role title is empty.,,"
    if exists_data > 0:
        exists_message = str(exists_data) + " Role already exists."
