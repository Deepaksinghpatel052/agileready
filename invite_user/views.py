            ...
            if request.method == "POST":
                ...
                for email_get in user_emails:
                    ...
                    if User.objects.filter(email=user_email).exists():
                        ...
                        try:
                            get_status = get_package_limit(request.session['org_id'],"users")
                            if get_status:    
                                ...
                                add_email = ArEmailSendToAdmin(User_Email=user_email,Email_Subjects="Invite New User",Ar_user=ar_users,ORG_ID=org_info,invited_by =corrent_user_info.user.username )
                                add_email.save()
                                #########################################################################
